...
# 1️⃣ Safely enter your API key (never stored in code)
...
# 3️⃣ Full KPI Diagnostics prompt (identical to your markdown)
prompt = """
...
4. **Validation Plan**
   - For each of the top 5 hypotheses, provide 2–3 *specific checks* to confirm or rule it out.
   - Example checks:
...
# 4️⃣ Stream the model’s response live
...
# 5️⃣ (Optional) Save the output to Markdown for your repo
with open("kpi_diagnostics_output.md", "w") as f:
    f.write(full_text)
